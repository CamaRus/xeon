(function($){
	...
	$('body').on('submit','form.-submit', function (e) {
		...
		if( stepCount ){
			...
			if( currentStep < stepCount ){
				otherSteps = $form.find('.acf-fields').first().children('.acf-field:not([data-current])');
				console.log($form.find('.acf-fields').first());
				otherSteps.find('.acf-error-message').remove();
				otherSteps.remove();
			}
		}
	...
	function acffSubmitForm( $form, $validator ){
		if( typeof otherSteps != 'undefined' ){
			$form.find('.acf-form-fields').append(otherSteps);
		}
		if( $validator.hasErrors() ) return;
		if( typeof currentStep != 'undefined' && currentStep < stepCount ){
			currentStep++;
